...
## Προσωρινή λύση πριν τον server

Αν θέλετε να δουλέψετε και οι δύο πριν στηθεί το Hetzner, υπάρχουν δύο δρόμοι.

### Α. Ο καθένας τρέχει το δικό του αντίγραφο

Χρειάζεται μόνο **Docker Desktop** και ο φάκελος του project — όχι Node, όχι Postgres:

```bash
git clone https://github.com/PanosLevedogiannis/argolidastay.git
cd argolidastay
cp .env.example .env
# βάλε PAYLOAD_SECRET: node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"

docker compose -f docker-compose.local.yml up -d --build
```

Άνοιξε `http://localhost:3000/admin` και φτιάξε λογαριασμό.

**Ο καθένας θα έχει ΞΕΧΩΡΙΣΤΑ δεδομένα.** Καλό για να δει κανείς πώς δουλεύει ή να δοκιμάσει, άχρηστο για κοινή δουλειά στο περιεχόμενο.

### Β. Κοινή βάση στο internet

Και οι δύο τρέχετε την εφαρμογή τοπικά, αλλά η βάση είναι μία και κοινή. Βλέπετε τα ίδια καταλύματα.

Άνοιξε δωρεάν Postgres σε [neon.tech](https://neon.tech) και βάλε στο `.env` **και των δύο**:

```
SHARED_DATABASE_URI=postgres://...η διεύθυνση που δίνει το Neon...
```
...
**Τι μοιράζεστε:** καταλύματα, περιοχές, άρθρα, αιτήματα, λογαριασμούς.
**Τι όχι:** τις φωτογραφίες — μένουν στον δίσκο του καθενός. Με 2-3 φωτογραφίες στην αρχή δεν είναι πρόβλημα· θα ξαναανέβουν μία φορά όταν στηθεί ο server.

> Και οι δύο λύσεις είναι προσωρινές. Το VPS κοστίζει περίπου 5 € τον μήνα και το στήσιμο θέλει μισή ώρα — αν σκοπεύετε να δουλέψετε σοβαρά στο περιεχόμενο, κάντε το κατευθείαν.
