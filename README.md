# ArgolidaStay

Κατάλογος καταλυμάτων στην Αργολίδα — Ναύπλιο, Τολό, Επίδαυρος, Πόρτο Χέλι, Ερμιόνη.

Ο κατάλογος φιλοξενεί καταλύματα τρίτων. Ο επισκέπτης βρίσκει το κατάλυμα και επικοινωνεί **απευθείας με τον ιδιοκτήτη** — το site δεν μεσολαβεί σε κρατήσεις ή πληρωμές.

## Πώς δουλεύει

Ο επισκέπτης αφήνει το τηλέφωνό του, ο ιδιοκτήτης ειδοποιείται με SMS και τον καλεί. Τιμές και κρατήσεις κανονίζονται μεταξύ τους, εκτός site.

Οι ιδιοκτήτες πληρώνουν **ετήσια συνδρομή** για να εμφανίζονται. Κάθε καταχώρηση έχει ημερομηνία λήξης και κρύβεται αυτόματα όταν περάσει — δεν χρειάζεται να το θυμηθεί κανείς.

Κάθε εκδήλωση ενδιαφέροντος καταγράφεται, είτε είναι συμπληρωμένη φόρμα είτε απλό πάτημα στο «Δες τηλέφωνο». Αυτό δεν είναι στατιστικό στολίδι: το «πήρες 47 αιτήματα φέτος» είναι το μόνο αντικειμενικό επιχείρημα όταν έρθει η ώρα της ανανέωσης.

## Τεχνολογίες

| | |
|---|---|
| Framework | Next.js 16 (App Router) + React 19 + TypeScript |
| CMS / πάνελ | Payload 3 — τρέχει μέσα στο ίδιο Next.js project |
| Βάση | SQLite τοπικά · Postgres στην παραγωγή |
| Styling | Tailwind CSS 4 |
| Εικόνες | Sharp — τα μεγέθη παράγονται αυτόματα στο ανέβασμα |

Το Payload δεν είναι ξεχωριστός server: ζει στο route group `src/app/(payload)` και μοιράζεται τη βάση με το frontend. Ένα project, ένα deploy.

## Ξεκίνημα

Χρειάζεται Docker για τη βάση (Docker Desktop σε Mac).

```bash
npm install
cp .env.example .env        # και συμπλήρωσε το PAYLOAD_SECRET
docker compose up -d        # ξεκινά την Postgres
npm run dev
```

Δημιουργία μυστικού κλειδιού:

```bash
node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"
```

- Site: http://localhost:3000
- Πάνελ διαχείρισης: http://localhost:3000/admin

Την πρώτη φορά το `/admin` ζητά να φτιάξεις τον πρώτο λογαριασμό.

## Δομή

```
src/
├── app/
│   ├── (frontend)/          # Το δημόσιο site
│   └── (payload)/           # Πάνελ + REST/GraphQL API (παράγεται από το Payload)
├── collections/             # Το μοντέλο δεδομένων — εδώ γίνονται οι αλλαγές
│   ├── Properties.ts        # Καταλύματα
│   ├── Enquiries.ts         # Αιτήματα επικοινωνίας + αποκαλύψεις τηλεφώνου
│   ├── Articles.ts          # Οδηγοί και άρθρα (SEO)
│   ├── Areas.ts             # Περιοχές
│   ├── Amenities.ts         # Παροχές
│   ├── Media.ts             # Φωτογραφίες
│   └── Users.ts             # Λογαριασμοί πάνελ
├── lib/
│   └── notifications.ts     # Αποστολή SMS στον ιδιοκτήτη
├── payload.config.ts        # Κεντρικές ρυθμίσεις, γλώσσες, βάση
└── payload-types.ts         # Παράγεται αυτόματα — μην το πειράζεις

scripts/
└── seed.mjs                 # Γεμίζει άδεια βάση με περιοχές και παροχές
```

Αν σβήσεις την τοπική βάση, ξαναγέμισέ την με:

```bash
SEED_PASSWORD="ο-κωδικός-σου" node scripts/seed.mjs
```

## Μοντέλο δεδομένων

**Καταλύματα** — τύπος, περιοχή, περιγραφές, χωρητικότητα, παροχές, φωτογραφίες, συντεταγμένες, ενδεικτική τιμή, στοιχεία επικοινωνίας ιδιοκτήτη, σύνδεσμοι προς Booking/Airbnb.

**Περιοχές** και **Παροχές** είναι ξεχωριστά collections και όχι ελεύθερο κείμενο μέσα στο κατάλυμα. Έτσι το «Ναύπλιο» γράφεται μία φορά, κάθε περιοχή μπορεί να αποκτήσει δική της σελίδα προορισμού για SEO, και τα φίλτρα αναζήτησης δουλεύουν πάνω σε σταθερή λίστα.

Τα καταλύματα έχουν **πρόχειρα (drafts)** με αυτόματη αποθήκευση: μια καταχώρηση δουλεύεται με την ησυχία της και εμφανίζεται στο site μόνο μετά τη δημοσίευση.

## Διγλωσσία

Ρυθμισμένη σε επίπεδο πεδίου. Όποιο πεδίο είναι `localized: true` αποθηκεύεται χωριστά στα ελληνικά και στα αγγλικά, και ο διαχειριστής αλλάζει γλώσσα από επιλογέα μέσα στη φόρμα.

Προεπιλογή τα ελληνικά. Αν λείπει αγγλική μετάφραση, σερβίρεται το ελληνικό κείμενο αντί για κενό.

> Το ίδιο το πάνελ (κουμπιά «Save», «Create New») παραμένει στα αγγλικά — το Payload δεν διαθέτει ελληνική μετάφραση διεπαφής. Όλες οι ετικέτες των πεδίων, που είναι και το μέρος που διαβάζει ο διαχειριστής, είναι στα ελληνικά.

## Βάση δεδομένων

**Postgres και στα δύο περιβάλλοντα** — τοπικά σε container, στον server δίπλα στην εφαρμογή. Η ίδια βάση παντού σημαίνει ότι δεν ανακαλύπτουμε διαφορές συμπεριφοράς την ώρα του deploy.

```bash
docker compose up -d      # ξεκινά
docker compose down       # σταματά
docker compose down -v    # σβήνει και τα δεδομένα
```

## Ανέβασμα στον server

Δες το **[DEPLOY.md](DEPLOY.md)** για τα αναλυτικά βήματα.

Σύνοψη: VPS με Ubuntu, Docker, και `docker-compose.prod.yml` που σηκώνει τρία container — εφαρμογή, Postgres, και Caddy που κάνει μόνο του το HTTPS.

Δύο πράγματα ζουν σε volume και επιβιώνουν κάθε ενημέρωσης: **η βάση** και **οι φωτογραφίες**. Αν οι φωτογραφίες έμεναν στον δίσκο του container, θα εξαφανίζονταν με το πρώτο deploy — είναι το συνηθέστερο λάθος σε τέτοια στησίματα.

## SMS προς τους ιδιοκτήτες

Όταν υποβληθεί αίτημα κλήσης, ένα hook στο `Enquiries` στέλνει SMS στον ιδιοκτήτη και καταγράφει το αποτέλεσμα στην ίδια εγγραφή.

**Χωρίς ρυθμισμένο πάροχο το σύστημα δουλεύει κανονικά** — το αίτημα αποθηκεύεται και φαίνεται στο πάνελ, απλώς δεν φεύγει SMS. Αυτό είναι σκόπιμο: αν πέσει ο πάροχος, ο επισκέπτης δεν πρέπει να δει σφάλμα και να φύγει.

Για ενεργοποίηση, στο `.env`:

```
SMS_PROVIDER=yuboto
SMS_API_KEY=το-κλειδί-σου
SMS_SENDER=ArgolidaStay
```

Ο κώδικας είναι γραμμένος με εναλλάξιμο πάροχο (`src/lib/notifications.ts`), οπότε η προσθήκη δεύτερου είναι μία συνάρτηση.

**Σημείωση κόστους:** τα SMS με ελληνικούς χαρακτήρες χωρούν 70 χαρακτήρες αντί για 160, γιατί απαιτούν διαφορετική κωδικοποίηση. Το μήνυμα είναι σκόπιμα σύντομο ώστε να μη χρεώνεται διπλό.

## SEO

Για τουριστικό site η επισκεψιμότητα έρχεται σχεδόν αποκλειστικά από το Google, οπότε τα εξής δεν είναι προαιρετικά:

- **`/sitemap.xml`** — παράγεται δυναμικά· κάθε νέο κατάλυμα μπαίνει αυτόματα, με τις δύο γλωσσικές εκδόσεις συνδεδεμένες.
- **`hreflang`** σε κάθε σελίδα. Χωρίς αυτό το Google βλέπει την ελληνική και την αγγλική ως δύο άσχετες σελίδες με παρόμοιο περιεχόμενο, και συχνά αγνοεί τη μία.
- **`canonical`** — τα φιλτραρισμένα αποτελέσματα (`?area=tolo&page=2`) δείχνουν στην κύρια σελίδα, ώστε να μη θεωρηθούν διπλότυπα.
- **Δομημένα δεδομένα** (`LodgingBusiness`, `Article`) — επιτρέπουν πλούσια αποτελέσματα με τιμή, τοποθεσία και εικόνα.

Τα φιλτραρισμένα αποτελέσματα **επιτρέπονται** στο `robots.txt`: αυτές ακριβώς οι σελίδες πιάνουν αναζητήσεις τύπου «βίλα Τολό».

## Εντολές

| | |
|---|---|
| `npm run dev` | Τοπικός server |
| `npm run build` | Build παραγωγής |
| `npm run generate:types` | Ξαναφτιάχνει το `payload-types.ts` μετά από αλλαγή σε collection |
| `npm run lint` | ESLint |

Μετά από κάθε αλλαγή σε `src/collections/` τρέξε `npm run generate:types`, ώστε το TypeScript να ξέρει τα νέα πεδία.

## Τι εκκρεμεί

- [x] Μοντέλο δεδομένων — καταλύματα, αιτήματα, συνδρομές, άρθρα
- [x] Αυτόματη απόκρυψη ληγμένων συνδρομών
- [x] Αποστολή SMS (με εναλλάξιμο πάροχο)
- [x] Σχεδίαση — γήινη παλέτα, σύστημα components
- [x] Σελίδες καταλύματος, περιοχής, άρθρου, καταχώρησης
- [x] Αναζήτηση και φίλτρα με σελιδοποίηση
- [x] Κουμπί «Δες τηλέφωνο» με καταγραφή
- [x] Φόρμες αιτημάτων (επισκέπτη και ιδιοκτήτη)
- [x] Χάρτης OpenStreetMap
- [x] Δίγλωσσο site (ελληνικά / αγγλικά) με επιλογέα γλώσσας
- [x] SEO — sitemap με hreflang, robots, canonical, δομημένα δεδομένα
- [ ] **Λογαριασμός σε ελληνικό πάροχο SMS** — χρειάζεται εγγραφή
- [ ] **Ανέβασμα στον server** — δες [DEPLOY.md](DEPLOY.md)
- [ ] Πραγματικές φωτογραφίες και περιεχόμενο
- [ ] Σελίδα «επικοινωνία», όροι χρήσης, πολιτική cookies
